...
public abstract class BigQueryWriter {
   ...
   */
  public BigQueryWriter(int retries, long retryWaitMs, ErrantRecordHandler errantRecordHandler, boolean skipInvalidRows) {
    ...
    do {
      ...
      try {
        ...
        if (failedRowsMap.isEmpty()) {
          ...
          if (skipInvalidRows) {
            logger.info("skipInvalidRows-exception: ", new BigQueryConnectException(table.toString(), failedRowsMap));
